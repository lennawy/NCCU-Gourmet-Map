...
public class DeleteFrame extends JFrame {
	...
	public DeleteFrame(String name, JTextArea printComment) {
		...
		button1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try (Connection conn = DriverManager.getConnection(url, username, password)) {
					String query = String.format("SELECT * FROM RateSurvey WHERE Restaurant = \'%s\' AND Name = \'%s\'", (String)restaurant.getSelectedItem(), name);
			...
			public void actionPerformed(ActionEvent e) {
				try (Connection conn = DriverManager.getConnection(url, username, password)) {
					...
					if (hasResultSet == 0) {
						...
						dispose();
						String query1 = String.format("SELECT * FROM RateSurvey WHERE Name = \'%s\'", name);
		...
		/*JButton button3 = new JButton("Back to home page");
