...
public class ViewRatingBoard extends JFrame{
	...
	public ViewRatingBoard() {
		...
		Image image = icon.getImage().getScaledInstance(20, 20, icon.getImage().SCALE_DEFAULT);
		...
		try (Connection conn = DriverManager.getConnection(url, username, password)) {
			...
			String query = "SELECT * FROM RateSurvey";
			boolean hasResultSet = stat.execute(query);
			if (hasResultSet) {
				ResultSet result = stat.getResultSet();
				printRating.setText(showResultSet(result));
				printRating.setFont(new Font("Times new Roman", Font.PLAIN, 16));
				result.close();
			}
		...
		button2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try (Connection conn = DriverManager.getConnection(url, username, password)) {
					String query = "SELECT * FROM RateSurvey WHERE Restaurant LIKE ?";
